...
public class MultiRegionSingleMarketDemandModel extends RegionalDemandModel implements
	...
	public void updateSupply() {
		...
		if (log.isDebugEnabled()) {
			...
			int[] pagentNumbers = new int[region.getPotentialAgents().size() + 1];
			for (Cell c : region.getCells()) {
				pagentNumbers[c.getOwner().getType().getSerialID() + 1]++;
			}
	...
	public void postTick() {
		log.info("Regional demand: " + demand.prettyPrint());
		log.info("Regional supply: " + totalSupply.prettyPrint());
		log.info("World residual: " + worldResidual.prettyPrint());
		log.info("World per-cell residual: " + perCellResidual.prettyPrint());
		log.info("Marginal utilities (regional): " + getMarginalUtilities().prettyPrint());
	}
